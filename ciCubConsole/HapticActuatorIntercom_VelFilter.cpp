...
for(int pkt=0;pkt<noActiveBoards+1;pkt++)
	...
	while(curSimTime<SIMULATION_TIME && !stopSim)
	{
		...
		switch (ch)
		{
			...
			case 'o':// this button sets offsets for no contact force and position
				desPos[0]=joint[0].pos;
				printf("\n Desired position activated \n");
					Destor[0]=joint[0].anInput1;
				r=SetDesiredPosition(desPos);
				r=SetDesiredTorque(Destor);	 // added on 1/2/13
			break;
	...
	/*		 if(((packetToReceive.content[13])& 0x80)>0) joint[boardNo-1].pidOutput= ((unsigned char)packetToReceive.content[12]+((packetToReceive.content[13]<<8)& 0xff00))-0xffff;
			...
			joint[boardNo-1].TarPos= (unsigned char)packetToReceive.content[38]+((packetToReceive.content[39]<<8)& 0xff00) 
								  +((packetToReceive.content[40]<<16)& 0xff0000)+((packetToReceive.content[41]<<24)& 0xff000000);
			joint[boardNo-1].TwinActPos= (unsigned char)packetToReceive.content[42]+((packetToReceive.content[43]<<8)& 0xff00) 
...
count=0;
	...
	//printf("\n MainMPos=%d  ThetaW=%d MainVel=%d  Xacceleration=%d Yacceleration=%d Zacceleration=%d \n", joint[0].TwinActPos,joint[0].TwinTargPos, joint[0].TwinVel, joint[0].Xaccleration, joint[0].Yaccleration, joint[0].Zaccleration);
	printf("\n DampPos=%d  DampVel=%d \n", joint[1].pos,joint[1].vel);
	printf("\n DampPos=%d  DampVel=%d DampPosTar=%d\n", joint[0].pos,joint[0].vel, desPos[0]);
	printf("\n TargetPos=%d TempTargetPos=%d\n", joint[0].TarPos, joint[0].TwinActPos);
