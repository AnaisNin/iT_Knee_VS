...
void updateLoop(void)
{
	...
	if (mainClock->getCurrentTimeSeconds() >= 0.001) //1ms loop
	{
		//	printf("Loop: %0.4f\n",Timer2);		
		//time1LoopHap = 0.0;
		//printf("desPosThred: %*d  sinefreq:%0.6f\n",7,desPosThred[0],sinefreq);	
	...
	//				startTorqueControl(1,1);
		...
		desPosThred[0]=(int)(157000.0*(1.0+sin(2.0*PI*sinefreq*Tsine)))+314000+FunctionOffset;
		r=SetDesiredPosition(desPosThred);
		//printf("desPosThred: %*d  sinefreq:%0.6f\n",7,desPosThred[0],sinefreq);	
		//printf("\n rampPos=%*d  rampSTEPthread=%*d   \n", 7,rampPos,7,rampSTEPthread);
		...
		if (rampON==ON)
		{
			...
			{
				rampPos+=rampSTEPthread;
				if ((rampPos+FunctionOffset)>posMAX)
				{
					rampDirection=REVERSE; 
					//break;
				}
			...
			else
			{
				...
				if ((rampPos-FunctionOffset)<posMIN)
				{
					rampDirection=FORWARD; 
				//	break;
				}
